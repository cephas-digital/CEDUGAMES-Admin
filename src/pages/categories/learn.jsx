...
const empty = { title: "", tag: "", description: "", imageUrl: "", sortOrder: 0 };
...
export default function Learn() {
  ...
  const open = (item = null) => {
    ...
    setForm(item ? { title: item.title, tag: item.tag || "", description: item.description || "", imageUrl: item.image_url, sortOrder: item.sort_order } : empty);
    setDialog(true);
  };
  const save = async (event) => {
    ...
    try {
      ...
      const body = { ...form, imageUrl, programId, parentId: parentId || null, itemType: type };
  ...
  const enter = item => { const index = order.indexOf(item.item_type); setParams({ program: programId, parent: item.id, type: index === 4 ? "questions" : order[index + 1] }); };
