...
export default function Resources() {
  ...
  const loadCategories = useCallback(() => axios.get("/admin/resource-categories").then(({ data }) => setCategories(data.categories || [])), []);
  const loadResources = useCallback(() => {
    setLoading(true);
    return axios.get("/admin/resources", { params: { page, search: search || undefined, categoryId: categoryId || undefined } })
      .then(({ data }) => { setResources(data.resources || []); setPagination(data.pagination || { page: 1, total: 0, totalPages: 1 }); })
  ...
  useEffect(() => { const timer = setTimeout(loadResources, 250); return () => clearTimeout(timer); }, [loadResources]);
  ...
  const uploadFiles = async (files) => {
    ...
    try {
      ...
      setPage(1); await Promise.all([loadResources(), loadCategories()]);
  ...
  const deleteCategory = async (category) => {
    ...
    try { await axios.delete(`/admin/resource-categories/${category.id}`); if (categoryId === category.id) setCategoryId(""); await Promise.all([loadCategories(), loadResources()]); toast.success("Category removed."); } catch (error) { toast.error(error.response?.data?.message || "Category could not be removed."); }
  };

  const deleteResource = async (resource) => {
    if (!window.confirm(`Delete “${resource.name}” permanently?`)) return;
    try { await axios.delete(`/admin/resources/${resource.id}`); toast.success("Resource deleted."); await Promise.all([loadResources(), loadCategories()]); } catch (error) { toast.error(error.response?.data?.message || "Resource could not be deleted."); }
  };
    ...
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="grid gap-6 lg:grid-cols-[250px_minmax(0,1fr)]">
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-6">
          ...
          <button type="button" onClick={() => setCategoryId("")} className={`mt-4 flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold ${!categoryId ? "bg-purple-100 text-purple-700" : "text-slate-600 hover:bg-slate-50"}`}><span>All resources</span><span>{pagination.total}</span></button>
